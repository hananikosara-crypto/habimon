...
export default function MonsterDisplay({ name, stage, totalPoints, level, courseType }: MonsterDisplayProps) {
  const table = courseType === '1year' ? EVOLUTION_TABLE_1YEAR : EVOLUTION_TABLE_3YEAR
  ...
  return (
    <div className="flex flex-col items-center bg-gradient-to-b from-emerald-50 to-white rounded-2xl p-6 border border-emerald-100 shadow-sm">
      <div className="flex gap-2 mb-4">
        ...
        <Badge className="bg-emerald-600 text-white text-xs">{stageName}</Badge>
      </div>

      <div className="text-8xl mb-3 select-none" style={{ filter: 'drop-shadow(0 4px 6px rgba(0,0,0,0.15))' }}>
        {emoji}
      </div>
